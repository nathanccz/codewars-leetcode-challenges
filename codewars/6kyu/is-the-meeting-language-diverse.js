...
function isLanguageDiverse(list)
    ...
    loop through list
        if result doesn't have language, set key to 0 
        result[language]++

    mostCommon <- find property in object w highest value

    loop through result
        if second el of curr > mostCommon[2] return false
...
function isLanguageDiverse(list) {
    ...
    for (let dev of list) {
        if (!hash[dev.language]) hash[dev.language] = 0
        hash[dev.language]++
    }

    const highestCount = Math.max(...Object.values(hash))
    
    for (let entry in hash) {
        if (highestCount > hash[entry] * 2) return false
    }
